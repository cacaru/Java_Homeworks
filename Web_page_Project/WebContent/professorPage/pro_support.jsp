...
<% 	
	...
	for( int i = 0; i< lid.size(); i++){
		proname.add( proDAO.returnPname( lpDAO.returnProID(lid.get(i)) ) );	
	}
	ResultSet notice = lecDAO.supportinfo(lid.get(0));
	if( notice != null){
		while( notice.next() ) {
			lno.add(notice.getString(1));
			title.add(notice.getString(2));
			date.add(notice.getString(3));
			context.add(notice.getString(4));
	...
	<div style="width:80%; float:right;">
			...
			<tbody>
				<tr height="15%">
					...
					<td width="40%" align="left">
						<a href="./write.jsp?ID=<%=ID %>&lid=<%=Lid %>&division=2" class="b_likeButton">등록</a>
					</td>
					<td align="left">
						과목선택
						<form action="./pro_support.jsp?ID=<%=ID %>" method="post">
		...
		<div class="container">
			...
			<table width="100%" class="table table-striped" style="text-align:center; border: 1px solid #dddddd">
				...
				<tbody>
					<%
						ArrayList<lectureboard> list = lecDAO.getsupportlist(pageNumber, Lid); 
						for(int j = 0; j < list.size(); j ++){
					%> 
					<tr>
						<!-- <td></td>  -->
						<td><%= list.get(j).getNo() %></td>
						<td><a href="view.jsp?ID=<%=ID %>&Lid=<%=Lid %>&no=<%=list.get(j).getNo() %>&division=2" > <%=list.get(j).getTitle() %></td>
			...
			<%	
				if( pageNumber != 1 ){
			%>
				<a href="./pro_notice.jsp?ID=<%=ID %>&lid=<%=Lid %>&pageNumber=<%=pageNumber-1%>" class="b_likeButton">이전</a>
			<%		
				} if ( lecDAO.nextsupPage(pageNumber + 1, Lid) ) {
			%>
				<a href="./pro_notice.jsp?ID=<%=ID %>&lid=<%=Lid %>&pageNumber=<%=pageNumber+1%>" class="b_likeButton">다음</a>
			<%		
				}
			%>
			</div>
